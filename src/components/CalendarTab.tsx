...
export const CalendarTab: React.FC<CalendarTabProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col px-4 sm:px-6 py-4 max-w-5xl mx-auto w-full select-none">
      {/* Calendar Header with Navigation */}
      <div className="flex items-center justify-between bg-white/90 rounded-2xl px-4 py-2.5 border border-orange-100 shadow-xs mb-4">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-['Fredoka',sans-serif] font-bold text-stone-800 text-base">
            {monthName}
          </span>
          <button
            onClick={nextMonth}
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={goToToday}
            className="px-3 py-1.5 text-xs font-bold rounded-xl bg-orange-100 text-orange-700 hover:bg-orange-200 transition"
          >
            Today
          </button>
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl bg-orange-500 hover:bg-orange-600 text-white shadow-xs transition"
          >
      ...
      {/* Main Responsive Grid: Calendar on Left, Agenda & History on Right on md+ screens */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
        {/* Left Column: Calendar Grid */}
        <div className="md:col-span-7 flex flex-col">
          {/* Weekday labels */}
          <div className="grid grid-cols-7 text-center text-[11px] font-extrabold text-stone-400 mb-1.5">
            <span>SUN</span>
            <span>MON</span>
            <span>TUE</span>
            <span>WED</span>
          ...
          {/* Month Days Grid */}
          <div className="grid grid-cols-7 gap-1.5 bg-white/80 p-3 rounded-2xl border border-orange-100/80 shadow-2xs">
            {days.map((item, index) => {
              if (!item) {
                return <div key={`empty-${index}`} className="h-12 rounded-xl" />;
              }
              ...
              return (
                <button
                  ...
                  className={`h-12 rounded-xl flex flex-col items-center justify-between p-1.5 transition relative ${
                    isSelected
                  ...
                  <div className="flex items-center gap-0.5">
                    {hasFocusLog && (
                      <span
                        ...
                        className={`text-[10px] ${isSelected ? 'opacity-90' : 'text-amber-500'}`}
                      >
                        🐾
                      </span>
                    )}
        ...
        {/* Right Column: Selected Day Agenda & Focus Summary */}
        <div className="md:col-span-5 flex flex-col overflow-y-auto pb-4">
          <div className="flex items-center justify-between mb-2.5">
            ...
            {totalMinutesFocusedOnDay > 0 && (
              <div className="flex items-center gap-1 text-xs font-bold text-amber-900 bg-amber-100 px-2.5 py-0.5 rounded-full border border-amber-200">
